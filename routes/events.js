...
router.post('/register/:eventId', authenticate, async (req, res) => {
    try {
        ...
        const studentId = req.user.id; // Extracted from the token by the `authenticate` middleware
        ...
        const qrData = {
            eventId,
            studentId,
            registrationId: `${eventId}-${studentId}`, // Unique registration ID
        };

        const qrCode = await QRCode.toDataURL(JSON.stringify(qrData));

        const qrCodePath = path.join(__dirname, `../qrcodes/${eventId}-${studentId}.png`);
        await QRCode.toFile(qrCodePath, JSON.stringify(qrData));

        // Cleanup QR code file after sending
